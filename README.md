...
```
docker build --build-arg BUILD_SCRIPT=build_skia --tag skia-builder .
```

Note: use `--no-cache` to rebuild
...
## Use rust-skia locally

* Build the docker container

```
docker build --build-arg BUILD_SCRIPT=build_skia_local --tag skia-builder-local .
```
